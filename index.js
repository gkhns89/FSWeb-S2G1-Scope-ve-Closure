...
/*Görev 1: macSkoru()
  ...
  skor1 sadece skorArtirici fonksiyonu içerisinden erişilerek değiştirilebilmekte ancak skor2 global olarak erişime açık durumdaır.
  2. Hangisi bir closure kullanmaktadır? Nasıl tarif edebilirsin? (yarınki derste öğreneceksin :) )
  skor1 closure kullanmaktadır, skorGuncelle fonksiyonu skorArtirici fonk. içerisine ulaşabilir durumda ancak skorArtirici fonk. skorGuncelle fonk. scope u içerisine erişemez durumdadır.
...
function takimSkoru() {
  const tSkor = Math.ceil(Math.random() * (25 - 10)) + 10 - 1;
...
function macSonucu(tSkoruFn, ceyrekSay) {
  let evSahibiSkor = 0;
  let konukTakimSkor = 0;

  for (let i = 0; i < ceyrekSay; i++) {
    evSahibiSkor += tSkoruFn();
    konukTakimSkor += tSkoruFn();
  }
  return {
...
function periyotSkoru(takimSkoru) {
  ...
  konukTakimSkor += takimSkoru();
  return {
    EvSahibi: evSahibiSkor,
    KonukTakim: konukTakimSkor,
  };
}
console.log("periyot skoru", periyotSkoru(takimSkoru));
